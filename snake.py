...
class Snake(Chainable):
    ...
    def __init__(self):
        self.direction = 'right' # default to moveing right
    ...
    def move(self):
        ...
        # Set each segments position to the next segments postion in line
        for i in range(segmentsLen - 1, 0, -1):
            self.segments[i].move( self.segments[i - 1].pos )

        # Finally set head segment to position of new direction move
...
def main():
    ...
    while(running):
        ...
        if checkCollision(headRect, appleRect):
            # Apple collected
            apple = Apple() # Make new people
            snake.addSegment()
